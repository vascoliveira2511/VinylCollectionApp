    ...
    const fetchUserPreferences = async () => {
      try {
        ...
        if (userRes.ok) {
          ...
          setShowGenreChart(
            userData.showGenreChart !== undefined
              ? userData.showGenreChart
              : true
          );
          setShowYearChart(
            userData.showDecadeChart !== undefined
              ? userData.showDecadeChart
              : true
          );
          ...
          if (vinyl.discogsId) {
            try {
              const discogsRes = await fetch(
                `/api/discogs/release/${vinyl.discogsId}`
              );
              if (discogsRes.ok) {
                const discogsData = await discogsRes.json();
                if (discogsData.country) {
                  countryCounts[discogsData.country] =
                    (countryCounts[discogsData.country] || 0) + 1;
                }
                ...
                console.error(
                  `Failed to fetch Discogs details for ${vinyl.discogsId}:`,
                  discogsRes.statusText
                );
              }
            } catch (error) {
              console.error(
                `Error fetching Discogs details for ${vinyl.discogsId}:`,
                error
              );
            }
          }
        ...
        <div className="window">
          ...
          <div className={styles.contentSection}>
            <div className={styles.statsHeader}>
              ...
              <p className={styles.totalRecordsText}>
              ...
              <div className={styles.checkboxGrid}>
                <label className={styles.chartCheckbox}>
                  <input
                    ...
                    onChange={() => setShowGenreChart(!showGenreChart)}
                  />
                ...
                <label className={styles.chartCheckbox}>
                  <input
                    ...
                    onChange={() => setShowYearChart(!showYearChart)}
                  />
                ...
                <label className={styles.chartCheckbox}>
                  <input
                    ...
                    onChange={() => setShowArtistChart(!showArtistChart)}
                  />
                ...
                <label className={styles.chartCheckbox}>
                  <input
                    ...
                    onChange={() => setShowCountryChart(!showCountryChart)}
                  />
                ...
                <label className={styles.chartCheckbox}>
                  <input
                    type="checkbox"
                    checked={showStyleChart}
                    onChange={() => setShowStyleChart(!showStyleChart)}
                  />
