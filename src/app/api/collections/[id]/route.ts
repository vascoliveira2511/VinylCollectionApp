...
export async function PUT(
  ...
  try {
    ...
    if (collection.count === 0) {
      return NextResponse.json(
        { error: "Collection not found or cannot be modified" },
        { status: 404 }
      );
    }
...
export async function DELETE(
  ...
  try {
    ...
    if (collection.isDefault || collection.isProtected) {
      return NextResponse.json(
        { error: "Cannot delete protected collection (Collection or Wantlist)" },
        { status: 400 }
      );
    }
