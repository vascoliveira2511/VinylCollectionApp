...
export async function GET(request: NextRequest) {
  try {
    ...
    if (storedUserId) {
      ...
      if (!token) {
        console.log(
          "No JWT token or stored user ID found, redirecting to login"
        );
        return NextResponse.redirect(
          new URL("/login?discogs_callback=true", request.url)
        );
      }
    ...
    const response = NextResponse.redirect(
      new URL("/profile?discogs=connected", request.url)
    );
