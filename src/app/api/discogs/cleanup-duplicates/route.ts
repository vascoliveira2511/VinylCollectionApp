...
export async function POST(request: NextRequest) {
  try {
    ...
    for (const vinyl of allVinyls) {
      const key = `${vinyl.artist.toLowerCase().trim()}-${vinyl.title
        .toLowerCase()
        .trim()}`;
      if (!groups.has(key)) {
        groups.set(key, []);
    ...
    for (const [key, vinyls] of Array.from(groups.entries())) {
      if (vinyls.length > 1) {
        try {
          ...
          const keepRecord = vinyls.reduce((best, current) => {
            ...
            const bestFields = Object.values(best).filter(
              (v) => v !== null && v !== ""
            ).length;
          ...
          if (duplicates.length > 0) {
            ...
            for (const duplicate of duplicates) {
              ...
              if (!mergedData.discogsId && duplicate.discogsId)
                mergedData.discogsId = duplicate.discogsId;
              if (!mergedData.imageUrl && duplicate.imageUrl)
                mergedData.imageUrl = duplicate.imageUrl;
              if (!mergedData.year && duplicate.year)
                mergedData.year = duplicate.year;
              if (!mergedData.label && duplicate.label)
                mergedData.label = duplicate.label;
              if (!mergedData.format && duplicate.format)
                mergedData.format = duplicate.format;
              if (!mergedData.catalogNumber && duplicate.catalogNumber)
                mergedData.catalogNumber = duplicate.catalogNumber;
              if (!mergedData.rating && duplicate.rating)
              ...
              if (!mergedData.condition && duplicate.condition)
                mergedData.condition = duplicate.condition;
              if (!mergedData.purchaseDate && duplicate.purchaseDate)
                mergedData.purchaseDate = duplicate.purchaseDate;
              if (!mergedData.purchasePrice && duplicate.purchasePrice)
                mergedData.purchasePrice = duplicate.purchasePrice;
              if (!mergedData.purchaseLocation && duplicate.purchaseLocation)
                mergedData.purchaseLocation = duplicate.purchaseLocation;
              if (!mergedData.country && duplicate.country)
                mergedData.country = duplicate.country;
          ...
          errors.push(
            `Failed to merge duplicates for "${key}": ${
