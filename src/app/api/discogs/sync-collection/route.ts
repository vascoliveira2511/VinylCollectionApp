...
export async function POST(request: NextRequest) {
  try {
    ...
    if (!user?.discogsAccessToken || !user?.discogsAccessTokenSecret || !user?.discogsUsername) {
    ...
    while (hasMorePages) {
      try {
          ...
          try {
            ...
            if (!existingVinyl) {
              const similarVinyls = await prisma.vinyl.findMany({
                where: {
                  ...
                  AND: [
                    ...
                    { title: { contains: release.title.split(' ').slice(0, 3).join(' '), mode: 'insensitive' } },
              ...
              if (similarVinyls.length > 0) {
                existingVinyl = similarVinyls.find(vinyl => 
                  vinyl.artist.toLowerCase().includes(artist.toLowerCase().split(',')[0].trim()) &&
                  vinyl.title.toLowerCase().includes(release.title.toLowerCase().slice(0, 20))
                ) || similarVinyls[0]
              }
            }
              ...
              if (!existingVinyl.discogsId) {
                await prisma.vinyl.update({
                  ...
                  data: {
                    ...
                    imageUrl: existingVinyl.imageUrl || release.cover_image || release.thumb || null,
                    genres: existingVinyl.genres === '[]' ? JSON.stringify(release.genres || []) : existingVinyl.genres,
                    label: existingVinyl.label || release.labels?.[0]?.name || null,
                    format: existingVinyl.format || release.formats?.[0]?.name || null,
                    catalogNumber: existingVinyl.catalogNumber || release.labels?.[0]?.catno || null,
            ...
            console.error('Error processing item:', item.basic_information.id, itemError)
            errorCount++
            errors.push(`Failed to sync release "${item.basic_information.title}": ${itemError instanceof Error ? itemError.message : 'Unknown error'}`)
          }
        }
        ...
        errors.push(`Failed to fetch page ${page}: ${pageError instanceof Error ? pageError.message : 'Unknown error'}`)
        break
      }
    }
